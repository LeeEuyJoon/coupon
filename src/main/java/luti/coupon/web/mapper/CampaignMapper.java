...
public class CampaignMapper {
	...
	public static CreateCampaignCommand toCommand(CreateCampaignRequest request) {
		List<CreateCampaignCommand.PolicyCommand> policies = request.policies().stream()
			.map(p -> CreateCampaignCommand.PolicyCommand.of(p.pointAmount(), p.quantity()))
			.toList();
		return CreateCampaignCommand.of(
			request.name(), request.description(),
			request.startAt(), request.endAt(), policies
		);
	}

	public static CampaignResponse toResponse(CampaignResult result) {
		List<CampaignResponse.PolicyResponse> policies = result.getPolicies().stream()
		...
		return new CampaignResponse(
			...
			result.getStartAt(), result.getEndAt(), result.getStatus(), policies
		);
	}
